...
void stats_add_to_total(struct stats_st * total,struct stats_st *thread_stats)
{
    ...
    stats_add_channel(&total->auth,&thread_stats->auth);
    stats_add_channel(&total->recs,&thread_stats->recs);
}
...
FILE *prom_fp = NULL;
    ...
    stats_channel(prom_fp,"auth",&stats->auth);
    stats_channel(prom_fp,"recs",&stats->recs);
    fclose(prom_fp);
}
